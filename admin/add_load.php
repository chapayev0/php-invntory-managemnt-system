...
mysqli_query($connection,$query_insert) or die("Unable to insert data into the tbl_load. ".mysqli_error());
...
for($x=1;$x<=$item_count;$x++){
	...
	if($_POST[$goods] != "" && $_POST[$goods_id] != "" && $_POST[$qty] != ""){
		...
		mysqli_query($connection,$query_goods_insert) or die("Unable to insert data into the tbl_load_item. ".mysqli_error());
	}
...
/*stock eken aduwenne mehemayi*/
	...
	for($y=1;$y<=$qty;$y++ ){
		...
		$item_qty_result=mysqli_query($connection,"SELECT MIN(st_id) AS id,st_remain_qty FROM tbl_stock WHERE p_name ='$_REQUEST[$goods]' AND st_status='0'") or die("Error :".mysqli_error());
	...
	mysqli_query($connection,"update tbl_stock set st_remain_qty='$update_qty' where st_id = '$id'")  or die("Error :".mysqli_error());
			...
			$up_tbl=mysqli_query($connection,"UPDATE tbl_stock SET st_remain_qty ='0', st_status='1' where st_id = '$id'") or die("Error :".mysqli_error());
			
			 $item_qty_result2=mysqli_query($connection,"SELECT MIN(st_id) AS id,st_remain_qty FROM tbl_stock WHERE p_name ='$_REQUEST[$goods]' AND st_status='0'") or die("Error :".mysqli_error());
			
			$item_qty_data1=mysql_fetch_array($item_qty_result2);
			$id2=$item_qty_data1['id'];
			$up_qty=$item_qty_data1['st_remain_qty']-$extra_qty;
			mysqli_query($connection,"update tbl_stock set st_remain_qty='$up_qty' where st_id = '$id2'")  or die("Error :".mysqli_error());
